...
class Raider:
    def __init__(self, target_miner, spawn_pos, from_camp=False, raider_type="normal"):
        ...
        self.max_hp = 95 if raider_type == "armored" else 42
        self.hp = self.max_hp
        self.damage = 18 if raider_type == "armored" else 14
        # ИСПРАВЛЕНО: радиус атаки такой же, как у обычных мобов при атаке HQ
        self.attack_range = scaled(40)
        raw_speed = max(1, scaled(0.65 if raider_type == "armored" else 1.0))
        ...
        self.reward = 45 if raider_type == "armored" else 25
        self.attacking_tower = None
        self.burn_timer = 0
        self.poison_timer = 0
        self.slow_timer = 0
        self.phys_armor = 0.35 if raider_type == "armored" else 0.05
        self.mag_armor = 0.12 if raider_type == "armored" else 0.05
        ...
        self.target_wire = None

    def find_nearest_wire(self, game):
        if not game.wire_cells:
            return None
        return min(
            game.wire_cells,
            key=lambda cell: math.hypot(
                self.pos[0] - (FIELD_OFFSET_X + cell[0] * CELL_SIZE + CELL_SIZE // 2),
                self.pos[1] - (FIELD_OFFSET_Y + cell[1] * CELL_SIZE + CELL_SIZE // 2)
            )
        )
    ...
    def move(self, towers, hq, enemies, allied_units, raider_camps, game):
        if not self.alive:
            return
        if self.attack_cooldown > 0:
            self.attack_cooldown -= 1

        if self.target_wire not in game.wire_cells:
            self.target_wire = self.find_nearest_wire(game)

        if self.target_wire is not None:
            tx = FIELD_OFFSET_X + self.target_wire[0] * CELL_SIZE + CELL_SIZE // 2
            ty = FIELD_OFFSET_Y + self.target_wire[1] * CELL_SIZE + CELL_SIZE // 2
            dist = math.hypot(self.pos[0] - tx, self.pos[1] - ty)
            if dist <= self.attack_range:
                if self.attack_cooldown <= 0:
                    game.damage_wire(self.target_wire, self.damage)
                    if self.target_wire not in game.wire_cells:
                        self.target_wire = None
                    self.attack_cooldown = self.attack_cooldown_max
                return
        ...
        if hq.hp > 0:
            if can_attack_hq_from_pos(self.pos):
                if self.attack_cooldown <= 0:
                    ...
                    self.attack_cooldown = self.attack_cooldown_max
                return
            self.move_towards((HQ_CENTER_X, HQ_CENTER_Y))
        return
...
class RaiderCamp:
    ...
    def update(self, game):
        ...
        if self.spawn_timer >= self.spawn_cooldown:
            self.spawn_timer = 0
            raider_type = "armored" if random.random() < 0.35 else "normal"
    ...
    def __init__(self, enemy_type, lane):
        ...
        stats = {
            ...
            "wurg": {"hp": 68, "phys": 0.0, "mag": 0.1, "speed": scaled(1.5), "rise": False, "evade": 0.12,
                     ...
                     "vulnerable_to": {"archer": 1.2, "cannon": 0.8, "mage": 0.9}},
            "hellbrute": {"hp": 480, "phys": 0.3, "mag": 0.05, "speed": scaled(0.19), "rise": False, "evade": 0,
                          "attack": MELEE, "color": (200, 0, 200), "atk_dmg": 32, "atk_range": scaled(50), "atk_cd": 110,
                          "vulnerable_to": {"archer": 0.8, "cannon": 1.2, "mage": 0.9}}
        }
        s = stats[enemy_type]
        self.max_hp = s["hp"]
        self.hp = self.max_hp
...
class SatanBoss(Enemy):
    def __init__(self, lane):
        ...
        self.mag_armor = 0.28
        self.vulnerable_to = {"archer": 0.8, "cannon": 1.2, "mage": 0.9}
        self.evade = 0
        ...
        self.stun_timer = 0
        self.stun_cooldown = 300
        self.stun_duration = 120
    ...
    def stun_tower(self, towers):
        ...
        candidates = []
        for tower in towers:
            if tower.hp <= 0 or tower.stun_timer > 0:
                continue
            if tower.tower_type == "miner" and not tower.attack_mode:
                continue
            if tower.damage > 0 and tower.range > 0:
                candidates.append(tower)

        if candidates:
            target = random.choice(candidates)
            target.stun_timer = self.stun_duration
            self.stun_timer = self.stun_cooldown
...
class Game:
    ...
    def can_place_wire(self, cell):
        return (
            0 <= cell[0] < GRID_WIDTH and
            0 <= cell[1] < GRID_HEIGHT and
            not is_hq_cell(cell)
        )

    def build_wire(self, cell):
        if not self.can_place_wire(cell):
            return False
        if cell in self.wire_cells:
            return True
        if self.money < WIRE_COST:
            return False
        self.money -= WIRE_COST
        self.wire_cells.add(cell)
        self.wire_hp[cell] = WIRE_MAX_HP
        return True

    def damage_wire(self, cell, damage):
        if cell not in self.wire_cells:
            return False
        self.wire_hp[cell] = self.wire_hp.get(cell, WIRE_MAX_HP) - damage
        if self.wire_hp[cell] <= 0:
            self.wire_cells.discard(cell)
            self.wire_hp.pop(cell, None)
            return True
        return False

    def is_near_active_miner(self, cell):
        for tower in self.towers:
            if tower.tower_type == "miner" and tower.hp > 0:
                if abs(tower.cell[0] - cell[0]) + abs(tower.cell[1] - cell[1]) <= 3:
                    return True
        return False

    def is_tower_powered(self, tower):
        if tower.tower_type == "miner":
            return True
        if tower.cell in self.wire_cells:
            return True
        if any(cell in self.wire_cells for cell in self.get_cardinal_neighbor_cells(tower.cell)):
            return True
        return self.is_near_active_miner(tower.cell)

    def draw_wires(self, surface):
        width = max(3, scaled(7))
        node_size = max(5, scaled(10))
        for x, y in self.wire_cells:
            cx = FIELD_OFFSET_X + x * CELL_SIZE + CELL_SIZE // 2
            cy = FIELD_OFFSET_Y + y * CELL_SIZE + CELL_SIZE // 2
            for dx, dy in ((0, -1), (1, 0), (0, 1), (-1, 0)):
                neighbor = (x + dx, y + dy)
                if neighbor in self.wire_cells:
                    end = (
                        cx + dx * CELL_SIZE // 2,
                        cy + dy * CELL_SIZE // 2
                    )
                    pygame.draw.line(surface, (118, 118, 118), (cx, cy), end, width)
                    pygame.draw.line(surface, (70, 70, 70), (cx, cy), end, max(1, width // 3))
            node_rect = pygame.Rect(cx - node_size // 2, cy - node_size // 2, node_size, node_size)
            pygame.draw.rect(surface, (135, 135, 135), node_rect)
            pygame.draw.rect(surface, (65, 65, 65), node_rect, max(1, scaled(1)))
    ...
    def generate_waves(self):
        waves = [
            [("orc", 36), ("wurg", 12)],
            [("orc", 36), ("wurg", 24), ("hellbrute", 4)],
            [("orc", 32), ("wurg", 24), ("hellbrute", 20)],
        ]
        return waves
    ...
    def start_wave(self, early=False):
        if self.wave <= self.total_waves and not self.wave_active and not self.wave_delay_active:
            ...
            if self.wave == self.total_waves:
                ...
                self.spawn_queue.insert(boss_index, ("satan", lane))
    ...
    def update(self):
        ...
        if self.raider_spawn_timer >= self.raider_spawn_cooldown:
            ...
            if miners:
                ...
                for i in range(2):
                    ...
                    raider_type = "armored" if random.random() < 0.3 else "normal"
        ...
        for raider in self.raiders[:]:
            if raider.alive:
                ...
                raider.move(self.towers, self.hq, self.enemies, self.allied_units, self.raider_camps, self)
                if not raider.alive:
                    if raider.hp <= 0:
    ...
    def draw_ui(self, surface):
        ...
        start_y = scaled(30)
        btn_gap = scaled(15)
        tower_types = ["archer", "mage", "cannon", "miner"]
        show_miner_mode = self.selected_tower and self.selected_tower.tower_type == "miner"
        num_btns = len(tower_types) + 4 + (1 if show_miner_mode else 0)
        ...
        wire_rect = pygame.Rect(right_x, btn_y, btn_width, btn_height)
        color = CYAN if self.wire_mode else LIGHT_GRAY
        pygame.draw.rect(surface, color, wire_rect)
        pygame.draw.rect(surface, BLACK, wire_rect, 2)
        surface.blit(font_small.render("Wire", True, BLACK), (wire_rect.x + 8, wire_rect.y + 8))
        surface.blit(font_small.render(f"${WIRE_COST}/cell", True, BLACK), (wire_rect.x + 8, wire_rect.y + btn_height//2))
        btn_y += btn_height + btn_gap
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if pos[0] >= SCREEN_WIDTH - SIDE_PANEL_WIDTH:
                ...
                if not panel_handled:
                    wire_rect = pygame.Rect(right_x, btn_y, btn_width, btn_height)
                    if wire_rect.collidepoint(pos):
                        game.wire_mode = not game.wire_mode
                ...
                if 0 <= cell[0] < GRID_WIDTH and 0 <= cell[1] < GRID_HEIGHT:
                    if game.wire_mode:
                        game.build_wire(cell)
                        game.wire_dragging = True
                    elif game.radar_mode:
        ...
        if event.type == pygame.MOUSEMOTION:
            if game.wire_dragging and game.wire_mode:
                pos = pygame.mouse.get_pos()
                if SIDE_PANEL_WIDTH <= pos[0] < SCREEN_WIDTH - SIDE_PANEL_WIDTH:
                    cell = cell_from_pos(pos)
                    if 0 <= cell[0] < GRID_WIDTH and 0 <= cell[1] < GRID_HEIGHT:
