...
class SatanBoss(Enemy):
    ...
    def take_damage(self, damage, damage_type):
        ...
        if self.phase == 1 and self.hp <= self.phase2_threshold:
            self.phase = 2
            self.speed = float(max(1, scaled(70)))
...
class Game:
    ...
    # ================== ИСПРАВЛЕННЫЙ MERGE (ПОДДЕРЖКА ЛЮБЫХ СВЯЗНЫХ КОНФИГУРАЦИЙ) ==================
    def merge_towers(self):
        # Удаляем из выделения все уничтоженные или отсутствующие башни
        self.merge_selection = [t for t in self.merge_selection if t in self.towers and t.hp > 0]
        ...
        # Проверка связности: все три клетки должны быть смежными (8-связность)
        cells_set = set(t.cell for t in self.merge_selection)
        if len(cells_set) != 3:  # если две башни на одной клетке – такого не должно быть
            return

        # BFS для проверки связности
        start = list(cells_set)[0]
        visited = set([start])
        queue = [start]
        while queue:
            cx, cy = queue.pop(0)
        ...
        if visited != cells_set:
            return  # не все клетки связны

        if base_type.startswith(('chimera', 'hell')):
            # Повторное слияние улучшенных башен
            cost = 300  # фиксированная стоимость
            if self.money < cost:
                return
            max_dmg = max(t.damage for t in self.merge_selection)
            max_rng = max(t.range for t in self.merge_selection)
            max_hp = max(t.max_hp for t in self.merge_selection)
            new_tower = Tower(base_type, self.merge_selection[0].cell)
            new_tower.damage = int(max_dmg * 1.5)
            new_tower.range = int(max_rng * 1.1)
            new_tower.max_hp = int(max_hp * 1.5)
            ...
            # Первое слияние: обычные башни → Химера
            cost = int(0.6 * TOWER_COST[base_type] * 3)
            if self.money < cost:
                return
            cell_for_new = self.merge_selection[0].cell
            for t in self.merge_selection:
                self.towers.remove(t)
            chimera_type = 'chimera_' + base_type
            self.towers.append(Tower(chimera_type, cell_for_new))
            self.money -= cost
    ...
    def update(self):
        ...
        # Удаляем уничтоженные башни и чистим выделение
        self.towers = [t for t in self.towers if t.hp > 0]
        self.merge_selection = [t for t in self.merge_selection if t in self.towers]
                ...
                if 0 <= cell[0] < GRID_WIDTH and 0 <= cell[1] < GRID_HEIGHT:
                    ...
                    if keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT]:
                        for t in game.towers:
                            if t.cell == cell and t.hp > 0:  # только живые
