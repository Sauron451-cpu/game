...
TOWER_COST = {
    ...
    "miner": 40, "barracks": 200  # Цена добытчика уменьшена до 40
}
TOWER_UPGRADE_COST = {
...
class Raider:
    def __init__(self, target_miner, spawn_pos):
        self.target_miner = target_miner
        self.pos = list(spawn_pos)
        # HP уменьшено в 2 раза: 102 / 2 = 51
        self.max_hp = 51
        self.hp = self.max_hp
        self.damage = 20
        self.attack_range = scaled(40)
        self.speed = scaled(2.0)
        self.base_speed = self.speed
        # Кулдаун атаки 25 секунд: 25 * 60 = 1500
        self.attack_cooldown_max = 1500
        self.attack_cooldown = 0
        self.alive = True
...
class RaiderCamp:
    def __init__(self, cell, pos):
        ...
        self.hp = self.max_hp
        self.spawn_timer = 0
        self.spawn_cooldown = 1800  # 30 секунд
    ...
    def update(self, game):
        ...
        if self.spawn_timer >= self.spawn_cooldown:
            self.spawn_timer = 0
            # Мародер появляется В ЛАГЕРЕ, а не в углу карты
            raider = Raider(None, (self.x, self.y))
...
class Enemy:
    def __init__(self, enemy_type, lane):
        ...
        # Все скорости уменьшены в 1.5 раза
        stats = {
...
class SatanBoss(Enemy):
    def __init__(self, lane):
        ...
        self.max_hp = 5000
        self.hp = self.max_hp
        # Скорость босса уменьшена в 1.5 раза: 40 / 1.5 ≈ 27
        raw_speed = max(1, scaled(27))
        self.speed = float(raw_speed)
        self.base_speed = float(raw_speed)
        self.phys_armor = 0.5
        self.mag_armor = 0.5
...
class Tower:
    def __init__(self, tower_type, cell):
        ...
        if tower_type == "miner":
            ...
            # Эффективность увеличена в 2 раза: cooldown 30 вместо 60
    ...
    def update(self, enemies, game, raider_camps, raiders):
        ...
        if self.tower_type == "miner":
            if self.mining and not self.attack_mode:
                ...
                # Эффективность увеличена в 2 раза: срабатывает каждые 30 кадров (0.5 сек)
...
class Headquarters:
    def __init__(self):
        ...
        # Атака HQ: стреляет по 2 целям
        self.attack_range = scaled(220)
        self.attack_damage = 30
        self.attack_cooldown_max = 60  # 1 секунда
        self.attack_cooldown = 0
        self.projectiles = []  # Список снарядов: [[target, [x, y]], ...]
    ...
    def find_targets(self, enemies, raiders, raider_camps):
        """Находит до 2 ближайших целей в радиусе атаки."""
        candidates = []
        for e in enemies:
            if e.alive:
        ...
        # Сортируем по расстоянию и берём 2 ближайшие
        candidates.sort(key=lambda x: x[0])
        return [c[1] for c in candidates[:2]]

    def update(self, game):
        # Пассивная генерация руды
...
class Game:
    def __init__(self):
        ...
        self.raider_spawn_cooldown = 600
        
        # Волны приходят раз в 50 секунд: 50 * 60 = 3000
        self.wave_interval = 3000
        self.wave_interval_timer = 0
        self.wave_delay = 420  # 7 секунд
    ...
    def start_wave(self, early=False):
        if self.wave <= 25 and not self.wave_active and not self.wave_delay_active:
    ...
    def update(self):
        ...
        if self.raider_spawn_timer >= self.raider_spawn_cooldown:
            ...
            if miners:
                ...
                raider = Raider(target, nearest_corner)
        ...
        if not self.wave_active and not self.wave_delay_active and self.first_wave_started:
            ...
            if self.wave_interval_timer >= self.wave_interval and self.wave <= 25:
    ...
    def post_wave_event(self):
        if self.waves_completed % 5 == 0 and self.wave < 25:
            if self.money >= 150:
                self.money -= 150
            else:
                if self.towers:
    ...
    def draw_ui(self, surface):
        ...
        if not self.first_wave_started:
            ...
            y_offset += scaled(50)
        elif not self.wave_active and self.wave <= 25:
        ...
        can_start = not self.wave_active and not self.wave_delay_active and self.wave <= 25 and not self.victory and not self.defeat
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if pos[0] >= SCREEN_WIDTH - SIDE_PANEL_WIDTH:
                ...
                if start_rect.collidepoint(pos):
                    can_start = not game.wave_active and not game.wave_delay_active and game.wave <= 25 and not game.victory and not game.defeat
                    if can_start:
