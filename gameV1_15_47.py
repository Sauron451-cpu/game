...
class RaiderCamp:
    ...
    def update(self, game):
        ...
        if self.spawn_timer >= self.spawn_cooldown:
            self.spawn_timer = 0
            raider_type = "armored" if random.random() < 0.5 else "normal"
...
class Game:
    ...
    def generate_veins(self):
        ...
        start_cells = [
            ...
            if not is_any_path_cell(cell) and not is_hq_cell(cell)
        ]
        if start_cells:
        ...
        if self.raider_spawn_timer >= self.raider_spawn_cooldown:
            ...
            if miners:
                ...
                for i in range(2):
                    ...
                    if self.wave > 2:
                        armored_chance = 0.25
                    elif self.wave == 2 and self.current_wave_spawn_total > 0:
                        spawned = self.current_wave_spawn_total - len(self.spawn_queue)
                        if spawned / self.current_wave_spawn_total >= 0.5:
                            armored_chance = 0.25
                    raider_type = "armored" if random.random() < armored_chance else "normal"
    ...
    def draw_tutorial_overlay(self, surface):
        ...
        steps = [
            ...
            (tower_rect, "Towers", "Pick Archer, Mage, or Cannon here, then place it on open land."),
